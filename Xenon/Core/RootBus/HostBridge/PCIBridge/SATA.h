...
// The COMMAND/DATA bit shall be cleared to zero if the transfer is data.
// Otherwise, the COMMAND/DATA bit shall be set to one.
#define IDE_INTERRUPT_REASON_CD 0x01
// The INPUT/OUTPUT bit shall be cleared to zero if the transfer is to the
// device. The INPUT/OUTPUT bit shall be set to one if the transfer is to the
// host.
 ...
 * It's completely unknown whether the Xenon Southbridge SATA is really based on
 * SiS technology. SCR seem to be SiS-like in PCI Config Space, but that should
 * be verified!
...
/*
 * Serial ATA provides an additional block of registers to control the interface
 * and to retrieve interface state information. There are 16 contiguous
 * registers allocated of which the first five are defined and the remaining 11
 * are reserved for future definition. Table 76 defines the Serial ATA Status
 * and Control registers. The registers start at offset that's specified by the
 * arch. Registers offsets are SCR_REG_BASE + RegNum * 4.
 */

/*
 * SStatus register:
 * The Serial ATA interface Status register - SStatus - is a 32-bit read-only
 * register that conveys the current state of the interface and host adapter.
 * The register conveys the interface state at the time it is read and is
 * updated continuously and asynchronously by the host adapter. Writes to the
...
/*
 * SError register:
 * The Serial ATA interface Error register - SError - is a 32-bit register that
 * conveys supplemental Interface error information to complement the error
 * information available in the Shadow Register Block Error register. The
 * register represents all the detected errors accumulated since the last time
 * the SError register was cleared (whether recovered by the interface of not).
 * Set bits in the error register are explicitly cleared by a write operation to
 * the SError register, or a reset operation. The value written to clear set
 * error bits shall have 1's encoded in the bit positions corresponding to the
 * bits that are to be cleared. Host software should clear the Interface SError
 * register at appropriate checkpoints in order to best isolate error conditions
 * and the commands they impact.
 */
#define SCR_ERROR_REG 1 // SCR[1] SError register.

/*
 * SControl register:
 * The Serial ATA interface Control register - SControl - is a 32-bit read-write
 * register that provides the interface by which software controls Serial ATA
 * interface capabilities. Writes to the SControl register result in an action
 * being taken by the host adapter or interface. Reads from the register return
...
/*
 * SActive register:
 * The SActive register is a 32-bit register that conveys the information
 * returned in the SActive field of the Set Device Bits FIS. If NCQ is not
 * supported, then the SActive register does not need to be implemented.
 */
#define SCR_ACTIVE_REG 3 // SCR[3] SActive register.

/*
 * SNotification register (Optional):
 * The Serial ATA interface notification register - SNotification - is a 32-bit
 * register that conveys the devices that have sent the host a Set Device Bits
 * FIS with the Notification bit set, as specified in section 10.3.6. When the
 * host receives a Set Device Bits FIS with the Notification bit set to one, the
 * host shall set the bit in the SNotification register corresponding to the
 * value of the PM Port field in the received FIS. For example, if the PM Port
 * field is set to 7 then the host shall set bit 7 in the SNotification register
 * to one. After setting the bit in the SNotification register, the host shall
 * generate an interrupt if the Interrupt bit is set to one in the FIS and
...
// Possible values:
#define SSTATUS_DET_NO_DEVICE_DETECTED                                         \
  0 // No device detected and Phy communication not established.
#define SSTATUS_DET_COM_NOT_ESTABLISHED                                        \
  1 // Device presence detected but Phy communication not established.
#define SSTATUS_DET_COM_ESTABLISHED                                            \
  3 // Device presence detected and Phy communication established.
#define SSTATUS_DET_DISABLED_OR_BIST                                           \
  4 // Phy in offline mode as a result of the interface being disabled or
    // running in a BIST loopback mode.

// SPD: The SPD value indicates the negotiated interface communication speed
// established.
#define SSTATUS_SPD_BITS 0xF0
#define SSTATUS_SPD_SHIFT 4
// Possible values:
#define SSTATUS_SPD_NO_SPEED                                                   \
  0 // No negotiated speed (device not present or communication not
    // established).
#define SSTATUS_SPD_GEN1_COM_SPEED                                             \
  1 // Generation 1 communication rate negotiated.
#define SSTATUS_SPD_GEN2_COM_SPEED                                             \
...
#define SSTATUS_IPM_INTERFACE_PARTIAL_PM_STATE                                 \
  2 // Interface in Partial power management state.
#define SSTATUS_IPM_INTERFACE_SLUMBER_PM_STATE                                 \
...
// ERR: The ERR field contains error information for use by host software in
...
/*
* C Non-recovered persistent communication or data integrity error: A
communication error that was not recovered occurred that is expected to be
persistent. Since the error condition is expected to be persistent the operation
need not be retried by host software. Persistent communications errors may arise
from faulty interconnect with the device, from a device that has been removed or
has failed, or a number of other causes.

* E Internal error: The host bus adapter experienced an internal error that
...
* I Recovered data integrity error: A data integrity error occurred that was
recovered by the interface through a retry operation or other recovery action.
This may arise from a noise burst in the transmission, a voltage supply
variation, or from other causes. No action is required by host software since
the operation ultimately succeeded, however, host software may elect to track
such recovered errors in order to gauge overall communications integrity and
potentially step down the negotiated communication speed.

* M Recovered communications error: Communications between the device and host
was temporarily lost but was re-established. This may arise from a device
temporarily being removed, from a temporary loss of Phy synchronization, or from
other causes and may be derived from the PHYRDYn signal between the Phy and Link
layers. No action is required by the host software since the operation
ultimately succeeded, however, host software may elect to track such recovered
errors in order to gauge overall communications integrity and potentially step
down the negotiated communication speed.

* P Protocol error: A violation of the Serial ATA protocol was detected. This
may arise from invalid or poorly formed FISes being received, from invalid state
transitions, or from other causes. Host software should reset the interface and
retry the corresponding operation. If such an error persists, the attached
device may have a design issue rendering it incompatible with the host bus
adapter.

* R Reserved bit for future use: Shall be cleared to zero.

* T Non-recovered transient data integrity error: A data integrity error
occurred that was not recovered by the interface. Since the error condition is
not expected to be persistent the operation should be retried by host software.
*/

// DIAG: The DIAG field contains diagnostic error information for use by
// diagnostic software in validating correct operation or isolating failure
// modes.The field is bit significant as defined in the following figure.
...
/*
* A Port Selector presence detected: This bit is set to one when COMWAKE is
received while the host is in state HP2: HR_AwaitCOMINIT. On power-up reset this
bit is cleared to zero. The bit is cleared to zero when the host writes a one to
this bit location.

* B 10b to 8b Decode error: When set to a one, this bit indicates that one or
more 10b to 8b decoding errors occurred since the bit was last cleared to zero.

* C CRC Error: When set to one, this bit indicates that one or more CRC errors
occurred with the Link layer since the bit was last cleared to zero.

* D Disparity Error: When set to one, this bit indicates that incorrect
disparity was detected one or more times since the last time the bit was cleared
to zero.

* F Unrecognized FIS type: When set to one, this bit indicates that since the
bit was last cleared one or more FISes were received by the Transport layer with
good CRC, but had a type field that was not recognized.

* I Phy Internal Error: When set to one, this bit indicates that the Phy
detected some internal error since the last time this bit was cleared to zero.

* N PHYRDY change: When set to one, this bit indicates that the PHYRDY signal
changed state since the last time this bit was cleared to zero.

* H Handshake error: When set to one, this bit indicates that one or more R_ERRP
handshake response was received in response to frame transmission. Such errors
may be the result of a CRC error detected by the recipient, a disparity or
10b/8b decoding error, or other error condition leading to a negative handshake
on a transmitted frame.

* R Reserved bit for future use: Shall be cleared to zero.

* S Link Sequence Error: When set to one, this bit indicates that one or more
Link state machine error conditions was encountered since the last time this bit
was cleared to zero. The Link layer state machine defines the conditions under
which the link layer detects an erroneous transition.

* T Transport state transition error: When set to one, this bit indicates that
an error has occurred in the transition from one state to another within the
Transport layer since the last time this bit was cleared to zero.

* W COMWAKE Detected: When set to one this bit indicates that a COMWAKE signal
was detected by the Phy since the last time this bit was cleared to zero.

* X Exchanged: When set to one this bit indicates that device presence has
changed since the last time this bit was cleared to zero. The means by which the
implementation determines that the device presence has changed is vendor
specific. This bit may be set to one anytime a Phy reset initialization sequence
occurs as determined by reception of the COMINIT signal whether in response to a
new device being inserted, in response to a COMRESET having been issued, or in
response to power-up.
*/
...
// DET: The DET field controls the host adapter device detection and interface
// initialization.
#define SCONTROL_DET_BITS 0xF
// Possible values:
#define SCONTROL_DET_NO_DEV                                                    \
  0 // No device detection or initialization action requested.
#define SCONTROL_DET_INIT                                                      \
  1 // Perform interface communication initialization sequence to establish
    // communication.This is functionally equivalent to a hard reset and results
    // in the interface being reset and communications reinitialized.
#define SCONTROL_DET_DISABLE                                                   \
  4 // Disable the Serial ATA interface and put Phy in offline mode.

// SPD: The SPD field represents the highest allowed communication speed the
// interface is allowed to negotiate when interface communication speed is
// established.
#define SCONTROL_SPD_BITS 0xF0
#define SCONTROL_SPD_SHIFT 4
// Possible values:
#define SCONTROL_SPD_NO_LIMIT 0 // No speed negotiation restrictions.
#define SCONTROL_SPD_GEN1                                                      \
  1 // Limit speed negotiation to a rate not greater than Gen 1 communication
    // rate.
#define SCONTROL_SPD_HEN2                                                      \
  2 // Limit speed negotiation to a rate not greater than Gen 2 communication
    // rate.

// IPM: The IPM field represents the enabled interface power management states
// that may be invoked via the Serial ATA interface power management
// capabilities.
#define SCONTROL_IPM_BITS 0xF00
#define SCONTROL_IPM_SHIFT 8
// Possible values:
#define SCONTROL_IPM_NO_RESTRICTION                                            \
  0 // No interface power management state restrictions.
#define SCONTROL_IPM_PARTIAL_PM_DISABLED                                       \
  1 // Transitions to the Partial power management state disabled.
#define SCONTROL_IPM_SLUMBER_PM_DISABLED                                       \
  2 // Transitions to the Slumber power management state disabled.
#define SCONTROL_IPM_ALL_PM_DISABLED                                           \
  3 // Transitions to both the Partial and Slumber power management states
    // disabled.

// The rest of the bits are Power-management related. If required i'll implement
// them.
...
/*
 * This structure is returned by the IDENTIFY_DEVICE and IDENTIFY_PACKET_DEVICE
 * commands.
 */
#pragma pack(push, 1)
