...
u32 PPCInterpreter::CRCompS(PPU_STATE *ppuState, u64 num1, u64 num2) {
  if (curThread.SPR.MSR.SF)
    ...
    return (
        CRCompS32(ppuState, static_cast<u32>(num1), static_cast<u32>(num2)));
}
...
void PPCInterpreter::ppcDebugLoadImageSymbols(PPU_STATE *ppuState,
  ...
  LOG_XBOX(Xenon, "Loaded: {} at address {:#x} - {:#x}", moduleName,
           Kdinfo.BaseOfDll, (Kdinfo.BaseOfDll + Kdinfo.SizeOfImage));
}
