...
#define D_STUB(name)                                                           \
  void PPCInterpreter_##name(PPU_STATE *hCore) {                               \
    return PPCInterpreter_known_unimplemented(#name, hCore);                   \
  }
#define D_STUBRC(name)                                                         \
  void PPCInterpreter_##name##x(PPU_STATE *hCore) {                            \
    return PPCInterpreter_known_unimplemented(#name "x", hCore);               \
  }
...
void PPCInterpreter_invalid(PPU_STATE *hCore) {
  ...
  std::string name = legacy_GetOpcodeName(thread.CI.opcode);

  LOG_CRITICAL(Xenon, "PPC Interpreter: {} found: data: {:#x}, address: {:#x}",
               name.data(), thread.CI.opcode, thread.CIA);
}
...
void PPCInterpreter_known_unimplemented(const char *name, PPU_STATE *hCore) {
  ...
  LOG_CRITICAL(
      Xenon,
      "PPC Interpreter: {} is not implemented! data: {:#x}, address: {:#x}",
      name, thread.CI.opcode, thread.CIA);
}
  ...
  fillTable(0x1F, 10, 1,
            {
                ...
                {0x1DC, GETRC(nand)},    {0x1F2, GET(slbia)},
                {0x1E7, GET(stvxl)},     {0x1E9, GETRC(divd)},
                {0x3E9, GETRC(divdo)},   {0x1EB, GETRC(divw)},
                {0x3EB, GETRC(divwo)},   {0x207, GET(lvlx)},
                {0x214, GET(ldbrx)},     {0x215, GET(lswx)},
                {0x216, GET(lwbrx)},     {0x217, GET(lfsx)},
                {0x218, GETRC(srw)},     {0x21B, GETRC(srd)},
                {0x227, GET(lvrx)},      {0x236, GET(tlbsync)},
                {0x237, GET(lfsux)},     {0x239, GET(mfsrin)},
                {0x253, GET(mfsr)},      {0x255, GET(lswi)},
                ...
                {0x353, GET(slbmfev)},   {0x356, GET(eieio)},
                {0x387, GET(stvlxl)},    {0x393, GET(slbmfee)},
