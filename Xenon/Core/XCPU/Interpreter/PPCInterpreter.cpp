...
void PPCInterpreter::ppcExecuteSingleInstruction(PPU_STATE *hCore) {
  ...
  if (thread.SPR.PIR == 0) {
    thread.lastRegValue = thread.GPR[11];
  }
...
void PPCInterpreter::ppcResetException(PPU_STATE *hCore) {
  ...
  LOG_INFO(Xenon, "[{}](Thrd{:#d}): Reset exception.", hCore->ppuName,
           (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.NIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
...
void PPCInterpreter::ppcDataStorageException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): Data Storage exception.", hCore->ppuName,
            (s8)hCore->currentThread);
...
void PPCInterpreter::ppcDataSegmentException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): Data Segment exception.", hCore->ppuName,
            (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.CIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
...
void PPCInterpreter::ppcInstStorageException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): Instruction Storage exception.",
            hCore->ppuName, (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.CIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
...
void PPCInterpreter::ppcInstSegmentException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): Instruction Segment exception.",
            hCore->ppuName, (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.CIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
...
void PPCInterpreter::ppcExternalException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): External exception.", hCore->ppuName,
            (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.NIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
...
void PPCInterpreter::ppcProgramException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): Program exception.", hCore->ppuName,
            (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.CIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
...
void PPCInterpreter::ppcDecrementerException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): Decrementer exception.", hCore->ppuName,
            (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.NIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
...
void PPCInterpreter::ppcSystemCallException(PPU_STATE *hCore) {
  ...
  LOG_TRACE(Xenon, "[{}](Thrd{:#d}): System Call exception.", hCore->ppuName,
            (s8)hCore->currentThread);
  thread.SPR.SRR0 = thread.NIA;
  thread.SPR.SRR1 =
      thread.SPR.MSR.MSR_Hex & (QMASK(0, 32) | QMASK(37, 41) | QMASK(48, 63));
  thread.SPR.MSR.MSR_Hex =
      thread.SPR.MSR.MSR_Hex &
      ~(QMASK(48, 50) | QMASK(52, 55) | QMASK(58, 59) | QMASK(61, 63));
  thread.SPR.MSR.MSR_Hex = thread.SPR.MSR.MSR_Hex | QMASK(0, 0) |
                           (thread.exceptHVSysCall ? 0 : QMASK(3, 3));
...
void PPCInterpreter::ppcInterpreterTrap(PPU_STATE *hCore, u32 trapNumber) {
      ...
      0x14) // DbgPrint, r3 = PCSTR stringAddress, r4 = int String Size.
  {
    ...
    for (int idx = 0; idx < strSize; idx++) {
      dbgString[idx] = MMURead8(hCore, thread.GPR[0x3] + idx);
    }
  ...
  if (trapNumber == 0x17) {
    ...
    ppcDebugLoadImageSymbols(hCore, thread.GPR[0x3], thread.GPR[4]);
  }
  if (trapNumber == 24) {
    ...
    ppcDebugUnloadImageSymbols(hCore, thread.GPR[0x3], thread.GPR[4]);
  }
