...
class PPCDecoder {
  class InstrInfo {
  public:
    constexpr InstrInfo(u32 v, instructionHandler p, instructionHandler pRc,
                        u32 m = 0)
        : value(v), ptr0(p), ptrRc(pRc), magn(m) {}

    constexpr InstrInfo(u32 v, const instructionHandler *p,
                        const instructionHandler *pRc, u32 m = 0)
        : value(v), ptr0(*p), ptrRc(*pRc), magn(m) {}
...
private:
  ...
  void fillTable(u32 mainOp, u32 count, u32 sh,
                 std::initializer_list<InstrInfo> entries) noexcept {
    if (sh < 11) {
