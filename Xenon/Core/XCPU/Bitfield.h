...
template <typename T, u64 N> class _bitfield {
public:
  // Types
  using bT = T;
  using vT = std::common_type_t<T>;
  using uT = std::make_unsigned_t<vT>;
  static constexpr bool canBePacked = N < (sizeof(s64) * 8 +
                                           (std::is_unsigned_v<vT> ? 1 : 0)) &&
                                      sizeof(vT) > sizeof(s64);
  using cT = std::conditional_t<
      canBePacked, std::conditional_t<std::is_unsigned_v<vT>, u64, s64>, vT>;
  // Bitsize
...
template <typename T, u64 I, u64 N> class bitfield : public _bitfield<T, N> {
public:
  // Types
  ...
  static constexpr cT extract(const T &data) noexcept {
    if constexpr (std::is_signed_v<T>) {
      ...
      return static_cast<cT>(
          static_cast<vT>(static_cast<uT>(data)
                          << (bitfield::bitmax - bitpos - N)) >>
          (bitfield::bitmax - N));
    } else {
      // Load unsigned value
      return static_cast<cT>((static_cast<uT>(data) >> bitpos) &
                             bitfield::vmask);
    }
  }
  // Bitfield insertion
  ...
  // Load bitfield value
  constexpr operator cT() const noexcept { return extract(this->data); }
  ...
  bitfield &operator++() { return *this = static_cast<cT>(*this + 1u); }
  cT operator--(s32) {
    cT result = *this;
    *this = static_cast<cT>(result - 1u);
    return result;
  ...
  bitfield &operator&=(cT right) {
    this->data &= static_cast<vT>(
        ((static_cast<uT>(right + 0u) & bitfield::vmask) << bitpos) |
...
class std::common_type<bitfield<T, I, N>, bitfield<T, I, N>>
    : public std::common_type<T> {};
template <typename T, u64 I, u64 N, typename T2>
class std::common_type<bitfield<T, I, N>, T2>
    : public std::common_type<T2, std::common_type_t<T>> {};
template <typename T, u64 I, u64 N, typename T2>
class std::common_type<T2, bitfield<T, I, N>>
    : public std::common_type<std::common_type_t<T>, T2> {};

// Field pack (concatenated from left to right)
template <typename F = void, typename... Fields>
class ControlField
    : public _bitfield<typename F::bT,
                       F::bitsize + ControlField<Fields...>::bitsize> {
public:
  // Types
  ...
  static constexpr vT data_mask() {
    return static_cast<vT>(F::data_mask() |
                           ControlField<Fields...>::data_mask());
  }
  // Extract all bitfields and concatenate
  static constexpr cT extract(const bT &data) {
    return static_cast<cT>(static_cast<uT>(F::extract(data))
                               << ControlField<Fields...>::bitsize |
                           ControlField<Fields...>::extract(data));
  }
  // Split bitfields and insert them
  static constexpr vT insert(cT value) {
    return static_cast<vT>(
        F::insert(value >> ControlField<Fields...>::bitsize) |
        ControlField<Fields...>::insert(value));
  }
  constexpr operator cT() const noexcept { return extract(this->data); }
...
template <> class ControlField<void> {
public:
  static constexpr u64 bitsize = 0;
  static constexpr u64 data_mask() { return 0; }
  template <typename T>
...
template <typename T, T V, u64 N> class FixedField : public _bitfield<T, N> {
public:
  using bT = typename FixedField::bT;
  using vT = typename FixedField::vT;
  // Return constant value
  static constexpr vT extract(const T &) {
    static_assert((V & FixedField::vmask) == V,
                  "FixedField<> error: V out of bounds");
    return V;
  }
  // Get value
  constexpr operator vT() const noexcept { return V; }
};
