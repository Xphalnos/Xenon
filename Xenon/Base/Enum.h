...
public:
  ...
  template <typename... Tx> Flags(T f, Tx... fx) { this->set(f, fx...); }

  template <typename... Tx> void set(Tx... fx) { m_bits |= bits(fx...); }

  void set(Flags flags) { m_bits |= flags.m_bits; }

  template <typename... Tx> void clr(Tx... fx) { m_bits &= ~bits(fx...); }

  void clr(Flags flags) { m_bits &= ~flags.m_bits; }

  template <typename... Tx> bool any(Tx... fx) const {
    return (m_bits & bits(fx...)) != 0;
  }

  template <typename... Tx> bool all(Tx... fx) const {
    const IntType mask = bits(fx...);
    return (m_bits & mask) == mask;
  }

  bool test(T f) const { return this->any(f); }
...
private:
  ...
  template <typename... Tx> static IntType bits(T f, Tx... fx) {
