...
bool ParseFilterRule(Filter &instance, Iterator begin, Iterator end) {
  ...
  if (level == Level::Count) {
    LOG_ERROR(Log, "Unknown log level in filter: {}",
              std::string_view(begin, end));
    return false;
  }
...
/// Macro listing all log classes. Code should define CLS and SUB as desired
...
bool Filter::IsDebug() const {
  return std::any_of(
      class_levels.begin(), class_levels.end(), [](const Level &l) {
